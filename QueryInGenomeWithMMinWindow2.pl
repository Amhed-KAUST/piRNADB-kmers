...
$num_args = $#ARGV + 1;
if ($num_args != 7) {
  print "\nUsage: QueryInGenomeWithMMinWindow2.pl Ref_fasta Query_fasta k-mer_length mm_in_window Pos-start_window Pos-end_window Out_file\n\n";
  ...
  print "\tFile to output number of matches.\n";
...
while($line=<op>){
chomp($line);
if(($line =~ m/^>/)){$string=$string."NNNNNNNNNNNNNNNN"; next;}
$string=$string.$line
}
close(op);
...
open(oout,">".$outF) or die "cannot write output file\n";
#print "\nReading sequences\n";
##my $con=0;
while($line=<op>){
##$con++;
